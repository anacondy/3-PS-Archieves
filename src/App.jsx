  ...
  <style>{`
    ...
    * {
    ...
    @media (max-width: 768px) {
      ...
      .mobile-p-safe { padding: env(safe-area-inset-top) env(safe-area-inset-right) env(safe-area-inset-bottom) env(safe-area-inset-left); }
    }
    
    /* Aspect ratio support for 16:9 and 20:9 devices */
    @media (min-aspect-ratio: 16/9) {
      .aspect-adaptive { aspect-ratio: 2/3; }
    }
...
const useSystemStats = () => {
  const [stats, setStats] = useState({ memory: '---', cpu: '---' });
    ...
    const measureStats = () => {
      ...
      if (now - lastMeasure >= 500) {
        ...
        if (performance.memory) {
          ...
          memoryStr = `~${navigator.deviceMemory}GB`;
        }

        // CPU approximation based on FPS drop from target 60fps
        const cpuLoad = Math.min(100, Math.max(0, Math.round((1 - fpsRef.current / 60) * 100 + Math.random() * 5)));
        const cpuStr = `${cpuLoad}%`;
      ...
      animationId = requestAnimationFrame(measureStats);
    };
...
const saveToLocalStorage = (games, tracks) => {
  try {
    ...
    const customTracks = tracks.filter(t => t.type === 'file' || t.id?.toString().startsWith('custom'));
    localStorage.setItem(TRACKS_STORAGE_KEY, JSON.stringify(customTracks));
  } catch (e) {
...
const loadFromLocalStorage = (defaultGames, defaultTracks) => {
  try {
    ...
    const customTracks = savedTracks ? JSON.parse(savedTracks) : [];
    const tracks = [...customTracks, ...defaultTracks.filter(t => !customTracks.some(ct => ct.id === t.id))];
    
    return { games, tracks };
  } catch (e) {
    console.warn('Failed to load from localStorage:', e);
    return { games: defaultGames, tracks: defaultTracks };
  }
};
        ...
        <div className="w-full px-4 h-12 flex items-center justify-between">
          ...
          <div className="hidden sm:flex gap-4 text-[10px] font-mono text-neutral-600 uppercase">
            <span className="flex items-center gap-1">
              ...
              MEM: <span className="text-blue-400">{systemStats.memory}</span>
            </span>
            <span className="flex items-center gap-1">
              <Cpu size={10} className="text-green-500" />
              CPU: <span className="text-green-400">{systemStats.cpu}</span>
            </span>
            <span className="flex items-center gap-1">
              FPS: <span className={`${systemStats.fps >= 55 ? 'text-green-400' : systemStats.fps >= 30 ? 'text-yellow-400' : 'text-red-400'}`}>{systemStats.fps || '---'}</span>
            </span>
          </div>
        </div>
      </header>
